...
/**
 * The Visual Studio Code brand mark, inlined so it works under the app's CSP and
 * offline — never hotlinked (same rule as HetznerLogo). Source: thesvg.org's
 * asset (github.com/glincker/thesvg, MIT). The logo itself is Microsoft's
 * trademark, used here nominatively: this button opens VS Code.
 *
 * Its mask/filters/gradient are referenced by id, and several task panels render
 * at once in Mission Control — so the ids are namespaced per instance (useId).
 * Shared literal ids would make every copy resolve to the first one in the DOM.
 */
export function VscodeLogo({ size = 16 }: { size?: string | number }) {
	const uid = useId().replace(/:/g, "");
	const mask = `vsc-mask-${uid}`;
	const shadowA = `vsc-shadow-a-${uid}`;
	const shadowB = `vsc-shadow-b-${uid}`;
	const sheen = `vsc-sheen-${uid}`;
	return (
		<svg
			width={size}
			height={size}
